...
class _AdminDashboardScreenState extends State<AdminDashboardScreen> with SingleTickerProviderStateMixin {
  ...
  // ============================================================
  // CARGA DE DATOS DESDE SERVICES (CONECTADOS A MYSQL)
  // ============================================================
  ...
  Future<void> _cargarPedidos() async {
    ...
    try {
      final res = await _contadorService.getReportesFinancieros();
      if (mounted) {
        setState(() {
          _pedidos = List<PedidoModel>.from(res['facturas'] ?? []);
          _ingresosTotales = (res['totalIngresos'] as num?)?.toDouble() ?? 0.0;
          _pedidosPendientes = res['pendientes'] ?? 0;
          _pedidosEntregados = res['pagadas'] ?? 0;
      ...
      appBar: AppBar(
        ...
        bottom: TabBar(
          ...
          tabs: const [
            ...
            Tab(icon: Icon(Icons.assignment_outlined), text: 'Pedidos'),
            Tab(icon: Icon(Icons.people_outline), text: 'Usuarios'),
            Tab(icon: Icon(Icons.swap_horiz_outlined), text: 'Devoluciones'),
          ],
        ),
      ),
      body: TabBarView(
        ...
        child: Column(
          ...
          children: [
            Wrap(
              ...
              runSpacing: 12,
              children: [
                _cardBox('Ingresos Totales', _formatearPrecio(_ingresosTotales), Icons.attach_money, Colors.teal),
                _cardBox('Total Pedidos', '${_pedidos.length}', Icons.assignment, Colors.blue),
                _cardBox('Pendientes', '$_pedidosPendientes', Icons.access_time_filled, Colors.amber),
                _cardBox('Pagados/Entregados', '$_pedidosEntregados', Icons.check_circle_outline, Colors.green),
              ],
            ),
            const SizedBox(height: 18),
                    ...
                    child: SingleChildScrollView(
                      ...
                      child: DataTable(
                        columns: const [
                          ...
                          DataColumn(label: Text('CAMBIAR ESTADO', style: TextStyle(fontWeight: FontWeight.bold))),
                          DataColumn(label: Text('ACCIONES', style: TextStyle(fontWeight: FontWeight.bold))),
                        ],
                        rows: _pedidos.map((ped) {
                          return DataRow(cells: [
                            DataCell(Text('#${ped.id}', style: const TextStyle(fontWeight: FontWeight.bold))),
                            DataCell(Text(ped.cliente)),
                            DataCell(Text(ped.fecha.split('T')[0])),
                            DataCell(Text(_formatearPrecio(ped.total), style: const TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF0047AB)))),
                            DataCell(
                              Container(
                                padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                                decoration: BoxDecoration(
                                  color: ped.estado.toLowerCase() == 'pagada' ? Colors.green.withValues(alpha: 0.15) : Colors.amber.withValues(alpha: 0.15),
                                  borderRadius: BorderRadius.circular(12),
                                ),
                                child: Text(ped.estado, style: TextStyle(color: ped.estado.toLowerCase() == 'pagada' ? Colors.green.shade800 : Colors.amber.shade900, fontWeight: FontWeight.bold, fontSize: 12)),
                              ),
                            ),
                            DataCell(
                              DropdownButton<String>(
                                value: ['Pendiente', 'Preparando', 'Enviado', 'Entregado', 'Cancelado'].contains(ped.estado) ? ped.estado : 'Pendiente',
                                items: const [
                                ...
                                onChanged: (nuevo) async {
                                  if (nuevo != null) {
                                    await _adminService.actualizarEstadoPedido(ped.id, nuevo);
                                    _cargarPedidos();
                                  }
                                },
                              ),
                            ...
                            DataCell(
                              IconButton(
                                ...
                                onPressed: () async {
                                  await _adminService.eliminarPedido(ped.id);
                                  _cargarPedidos();
                                },
                              ),
                            ),
